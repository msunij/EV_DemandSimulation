...
#Simulation starts on 1st Januvary 2019(Tuesday) 
def is_weekend(minutes_passed):
    d = datetime(2019,1,1)+timedelta(minutes=minutes_passed)
    if d.weekday()>4:
        return True
    else:
        return False
...
class EVClass:
    ...
    def monitor_demand(self,env):
        i = 2
        while True:
            minutes_passed = env.now
            print("Power demand by {} on {} = {}".format(self.name,format_minutes(minutes_passed),self.power_demand))
            sheet.cell(row=i,column=1).value = format_minutes(minutes_passed)
            sheet.cell(row=i,column=int(self.name[5:])).value = self.power_demand
            i+=1
            yield env.timeout(60)

## main starts here
wb = openpyxl.load_workbook('output_template.xlsx')
sheet = wb.active
